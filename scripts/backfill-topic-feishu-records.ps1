...
$script = Join-Path $root "scripts\backfill_topic_feishu_records.py"
$normalizedTopicIds = @()
foreach ($rawId in $TopicId) {
    $normalizedTopicIds += ($rawId -split "," | ForEach-Object { $_.Trim() } | Where-Object { $_ })
}

$argsList = @($script)
foreach ($id in $normalizedTopicIds) {
    $argsList += "--topic-id"
    $argsList += $id
}
if ($DryRun.IsPresent) {
